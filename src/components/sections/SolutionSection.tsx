...
const SolutionSection: React.FC = () => {
  ...
  const featureDescriptions = t('solution.featureDescriptions', { returnObjects: true }) as string[]
      ...
      <Container>
        <Reveal>
          <div className="flex flex-col gap-4">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-cyan-500">{t('solution.title')}</p>
        ...
        <div className="mt-12">
          ...
          <Stagger className="mt-6 grid auto-rows-fr gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, index) => (
              <Reveal key={`solution-feature-${index}`} withParent y={18} delay={index * 0.04}>
                <Card
                  className="card-border-strong flex h-full items-start gap-3"
                  tone={tones[index % tones.length]}
                >
                  <CheckCircle2 className="mt-1 text-emerald-400 transition-transform duration-300 group-hover:rotate-6 motion-reduce:transform-none" size={18} />
                  <div>
                    <h4 className="text-lg font-semibold text-slate-900 dark:text-white">{feature}</h4>
